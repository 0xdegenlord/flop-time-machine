...
export function parseArchiveSearchParams(
  ...
  if (query.length > 200 || /[\u0000-\u001f\u007f]/.test(query)) {
    errors.push(
      "Search must be 200 characters or fewer and contain no control characters.",
    );
  }

  if (did.length > 500 || /[\u0000-\u001f\u007f]/.test(did)) {
    errors.push(
      "DID must be 500 characters or fewer and contain no control characters.",
    );
  }
...
export function buildArchiveHref(
  ...
  if (filters.did) {
    params.set("did", filters.did);
  }

  if (filters.epochNumber) {
    params.set("epoch", String(filters.epochNumber));
  }
  ...
  if (filters.throughDate) {
    params.set("through", filters.throughDate);
  }
